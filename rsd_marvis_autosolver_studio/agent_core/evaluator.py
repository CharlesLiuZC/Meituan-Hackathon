"""Official-like evaluator (stdlib only), adapted from the Meituan-Hackathon
agent/evaluator.py. Lower result_key is better: valid > coverage > penalty."""
...
def evaluate_output(input_text: str, solution: Any) -> dict[str, Any]:
    ...
    if not isinstance(solution, list):
        return invalid_result(len(all_tasks))
    ...
    duplicate_tasks = sorted(t for t, c in task_count.items() if c > 1)
    duplicate_couriers = sorted(c for c, n in courier_count.items() if n > 1)
...
def invalid_result(total_tasks: int) -> dict[str, Any]:
    return {
        "valid": False,
        "covered_tasks": 0,
        "total_tasks": total_tasks,
        "missing_tasks": total_tasks,
        "total_score": INF,
        "penalty_score": INF,
        "parallel_penalty_score": INF,
        "duplicate_tasks": [],
        "duplicate_couriers": [],
        "unknown_tasks": [],
...
def result_key(result: dict[str, Any]) -> tuple:
    ...
    return (
        ...
        total - covered,
        float(result.get("penalty_score", INF)),
        float(result.get("parallel_penalty_score", result.get("penalty_score", INF))),
        float(result.get("total_score", INF)),
    )
